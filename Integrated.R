...
load("/data/Integrate/Tedd.4_NeonatalAdrenalGland/Tedd.4.Rda")

Tedd.4.3 <- SCTransform(Tedd.4, verbose = FALSE,method = "glmGamPoi")

Tedd.4.3$stim <- "Tedd.4"

load("/data/Integrate/Tedd.5_FetalAdrenalGland/Tedd.5.Rda")

Tedd.5 <- SCTransform(Tedd.5, verbose = FALSE,method = "glmGamPoi")

Tedd.5$stim <- "Tedd.5"

gene <- Reduce(intersect,  list(Tedd.4.3 = rownames(Tedd.4.3), Tedd.5 = rownames(Tedd.5)))

save(gene, file = "gene.Rda")

object.list <- list(Tedd.4.3 = Tedd.4.3,Tedd.5 = Tedd.5)

features <- SelectIntegrationFeatures(object.list = object.list, nfeatures = 3000)

features

merged.data <- merge(Tedd.4.3, c(Tedd.5),project = "Implantation")
